...
-- Using the actual 'profiles' table structure
...
-- Create RLS policies using the actual 'profiles' table
...
-- Admin actions - allow all authenticated users to view, only admins to insert
CREATE POLICY "Admin actions are viewable by everyone" ON public.admin_actions
    FOR SELECT USING (true);

CREATE POLICY "Admin actions are insertable by admins" ON public.admin_actions
...
-- Feature flags - allow all authenticated users to view, only admins to edit
CREATE POLICY "Feature flags are viewable by everyone" ON public.feature_flags
    FOR SELECT USING (true);

CREATE POLICY "Feature flags are editable by admins" ON public.feature_flags
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM public.profiles 
            WHERE id = auth.uid() AND role = 'admin'
...
DO $$
BEGIN
    RAISE NOTICE 'Final admin dashboard migration completed successfully!';
    ...
    RAISE NOTICE '- RLS policies using profiles table';
    RAISE NOTICE '';
    RAISE NOTICE 'Your admin dashboard is now ready!';
    RAISE NOTICE 'Go to /admin in your app and click "Control Center"';
END $$;
